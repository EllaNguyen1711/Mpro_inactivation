...
def distance_Arg_Glu(pdb, traj):
    ...
    dis.run()
    dis = np.array([np.min(dis.result[:, 0:4], 1)] +
                   [np.min(dis.result[:, 4:8], 1)])
    dis = dis.T
    return dis
...
def distance_Gln_Arg(pdb, traj):
    ...
    dis.run()
    dis = np.array([np.min(dis.result[:, 0:2], 1)] +
                   [np.min(dis.result[:, 2:4], 1)])
    dis = dis.T
    return dis
...
def Tyr_Xaa_His(pdb, traj):
    sim = mda.Universe(pdb, traj)
    Tyr_OH = sim.select_atoms('resid 161 and name HH')
    His163_ND1 = sim.select_atoms('resid 163 and name ND1')
    dis1 = DistanceAnalysis(Tyr_OH, His163_ND1)
    dis1.run()
    dis = dis1.result[:]
    return dis


def Glu166_His172(pdb, traj):
    sim = mda.Universe(pdb, traj)
    Glu166 = sim.select_atoms('resid 166 and (name OE1 or name OE2)')
    His172 = sim.select_atoms('resid 172 and name NE2')
    His = His172[0] + His172[0] + His172[1] + His172[1]
    dis1 = DistanceAnalysis(Glu166, His)
    dis1.run()
    dis = np.array([np.min(dis1.result[:, 0:2], 1)] +
                   [np.min(dis1.result[:, 2:4], 1)])
    dis = dis.T
    return dis
